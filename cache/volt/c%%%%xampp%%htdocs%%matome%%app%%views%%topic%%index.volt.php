		...
		<div class="list-group">
			...
			<div class="list-group-item no-border no-background-color have-bottom-line comment-items" data-link='<?php echo $this->url->get('topic/koMatome/' . $comment->comment_id); ?>'>
				...
				<?php if ($comment->url_comment != null) { ?>
					<h6><?php echo $comment->url_comment; ?></h6>
					<?php echo $this->tag->linkTo(array($comment->url_comment, 'class' => 'font-gray md-size', 'リンクをオープンする', false)); ?>
				...
				<?php } elseif ($comment->video_url != null) { ?>
					...
					<?php } elseif ($comment->video_type == 'website') { ?>
						...
						<?php } ?>
						<?php echo $this->tag->linkTo(array($comment->video_url, 'class' => 'font-gray md-size', 'リングを', false)); ?>
					<?php } else { ?>
						<div class="col-xs-12">
							<a href="javascript:void(0);" class="thumbnail">
			...
			<?php } ?>
				...
				<?php if ($auth != null) { ?>
					<li id="fav_controller">
						<?php if ($is_fav == false) { ?>
							<a href="javascript:void(0);" class="js-add-fav" data-id='<?php echo $topic->page_id; ?>' >
								<i class="pe-7s-star "></i>
								<p class="sm-size">お気に入り追加</p>
							</a>
						<?php } else { ?>
							<a href="javascript:void(0);" class="js-del-fav" data-id='<?php echo $topic->page_id; ?>'>
